...
export const ReportPreview: React.FC<ReportPreviewProps> = ({ biopsies, activeField }) => {
  const generateReportText = (): string => {
    ...
    // Group biopsies by stain type
    const stomachBiopsies = biopsies.filter(b => b.location === BiopsyLocation.Mide);
    const duodenumBiopsies = biopsies.filter(b => b.location === BiopsyLocation.Duodenum);
    const synaptophysinBiopsies = biopsies.filter(b => b.stains?.synaptophysin);

    if (stomachBiopsies.length > 0 || duodenumBiopsies.length > 0 || synaptophysinBiopsies.length > 0) {
      report += '\n\nHistokimyasal yöntemle:';
      
      // Add stomach stains
      if (stomachBiopsies.length > 0) {
        const biopsyNumbers = stomachBiopsies.map((_, i) => i + 1).join(', ');
        report += `\n${biopsyNumbers} - nolu örnekte Mide mukozasında intestinal metaplaziyi değerlendirmek için ${stomachBiopsies.length} adet PAS+AB, Helikobakter Pilori değerlendirmek için ${stomachBiopsies.length} adet Warthin Starry`;
      }

      // Add duodenum stains
      if (duodenumBiopsies.length > 0) {
        const biopsyNumbers = duodenumBiopsies.map((b, i) => biopsies.findIndex(bx => bx.id === b.id) + 1).join(', ');
        if (stomachBiopsies.length > 0) report += '\n';
        report += `${biopsyNumbers} - nolu örnekte Duedonum mukozasında villus ve silyalı epiteli değerlendirmek için ${duodenumBiopsies.length} adet PAS`;
      }

      // Add synaptophysin if used
      if (synaptophysinBiopsies.length > 0) {
        report += '\nİmmunhistokimyasal yöntemle:';
        report += '\nNöroendokrin hücre artışını değerlendirmek için 1 adet Sinaptofizin';
      }
      ...
      if (customStains.length > 0) {
        report += ` ${customStains.join(', ')} boyası yapılmıştır.`;
      }
    }
  ...
  const generateBiopsyReport = (biopsy: Biopsy, index: number, currentCount: number, totalCount: number): string => {
    ...
    if (biopsy.location === BiopsyLocation.Ozefagus) {
      ...
      if (hasActiveFindings) {
        ...
        lines.push(
          shouldHighlight('finding-hp') ? `<mark>${hpLine}</mark>` : hpLine,
          shouldHighlight('finding-intestinalMetaplasia') ? `<mark>${imLine}</mark>` : imLine
        );
      }
  ...
  const getStomachFeatureText = (key: string): string => {
    switch (key) {
      ...
      case 'activeLymphoidFollicle': return 'Germinal merkezi aktif lenfoid folikül vardır';
