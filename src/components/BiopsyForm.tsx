...
export const BiopsyForm: React.FC<BiopsyFormProps> = ({
  ...
  const handleLocationClick = (loc: string) => {
    ...
    // For Duodenum/Bulbus, handle like radio buttons
    ...
    // For other locations, keep the existing behavior
    const currentMainLocation = biopsy.subLocation.split(', ')[0];
    
    if (currentMainLocation === loc) {
      onUpdate({
        ...biopsy,
        subLocation: biopsy.subLocation.split(', ').slice(1).join(', '),
      });
    } else {
      const newLocation = loc;
      onUpdate({
        ...biopsy,
        subLocation: biopsy.subLocation ? `${newLocation}, ${biopsy.subLocation}` : newLocation,
      });
    }
  };
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {LocationOptions[biopsy.location].locations.map(loc => {
              const isSelected = biopsy.location === BiopsyLocation.Duodenum 
                ? biopsy.subLocation === loc 
                : biopsy.subLocation.split(', ')[0] === loc;
